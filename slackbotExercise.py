...
def selectExerciseAndStartTime(bot):
    ...
    lottery_announcement = "Next lottery for " + exercise["name"].upper() + " in " + str(minute_interval) + (" minutes" if minute_interval != 1 else " minute")
...
def selectNextTimeInterval(bot):
    return random.randint(bot.min_countdown, bot.max_countdown)*60
...
def main():
    ...
    # Setup the kill signal listener
    ...
    # Run the bot until stopped or office hours are done
    try:
        while True:
            if isOfficeHours(bot):
                ...
                if not bot.debug:
                     # respond and exit
                    requests.post(bot.post_URL, data="End of workday! Stopping bot")
                    saveUsers(bot)
                break
